...
def build_data_cv(data_folder, cv=10, clean_string=True):
    ...
    """
    revs = []
    ### CSV where each row contains a sequence of comments following a post, 
    # a set of responses to the last comment in that sequence, and sarcastic/non-sarcastic labels for those responses. 
    # The format is "post_id comment_id ... comment_id|response_id ... response_id|label ... label", 
    ...
    # The first entry always contains a post_id and 0 or more comment_ids. 
    # The second and third entries have the same number of elements, with the first response_id corresponding to the first label and so on.
    ...
    # dictionary in JSON format containing text and metadata for each comment in {comment_id: data} format
    ...
    for line in train_data: 
        rev = []
        # Sarcastic True or False:
        label_str = line[2]
        if( label_str == 0):
            label = [1, 0]
        else:
            label = [0, 1]
        # Comment text corresponding to current post
...
if __name__=="__main__":  
    ...
    # Number of words of post containing most words of all posts (max number of words in a single post)
